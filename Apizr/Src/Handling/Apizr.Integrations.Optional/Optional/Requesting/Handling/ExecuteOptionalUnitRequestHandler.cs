...
namespace Apizr.Optional.Requesting.Handling
{
    ...
    public class ExecuteOptionalUnitRequestHandler<TWebApi, TModelData, TApiData> : ExecuteOptionalUnitRequestHandlerBase<TWebApi, TModelData, TApiData, ExecuteOptionalUnitRequest<TWebApi, TModelData, TApiData>, IApizrRequestOptions, IApizrRequestOptionsBuilder>
    {
                ...
                switch (request.ExecuteApiMethod)
                {
                    ...
                    case Expression<Func<IApizrRequestOptions, TWebApi, TApiData, Task>> executeApiMethod:
                        return await request
                            ...
                            .MapAsync(async _ =>
                            {
                                await WebApiManager.ExecuteAsync<TModelData, TApiData>((options, api, apiData) => executeApiMethod.Compile().Invoke(options, api, apiData), request.ModelRequestData, request.OptionsBuilder);
    ...
    /// </summary>
    /// <typeparam name="TWebApi">The web api type</typeparam>
    public class ExecuteOptionalUnitRequestHandler<TWebApi> : ExecuteOptionalUnitRequestHandlerBase<TWebApi, ExecuteOptionalUnitRequest<TWebApi>, IApizrRequestOptions, IApizrRequestOptionsBuilder>
    {
        public ExecuteOptionalUnitRequestHandler(IApizrManager<TWebApi> webApiManager) : base(webApiManager)
        {
        }
                ...
                switch (request.ExecuteApiMethod)
                {
                    ...
                    case Expression<Func<IApizrRequestOptions, TWebApi, Task>> executeApiMethod:
                        return await request
                            ...
                            .MapAsync(async _ =>
                            {
                                await WebApiManager.ExecuteAsync((options, api) => executeApiMethod.Compile().Invoke(options, api), request.OptionsBuilder);
